...
> *Nicolò Gusmeroli, Timotej Hrga, Borut Lužar, Janez Povh, Melanie Siebenhofer, and Angelika Wiegele.*  
> **"BiqBin: A Parallel Branch-and-Bound Solver for Binary Quadratic Problems with Linear Constraints"**  
> *ACM Trans. Math. Softw. 48, 2, Article 15 (June 2022), 31 pages.*  
...
make run
```
Or the following for to run all g05_* instances
```bash
make run-all
```
---
...
Can be run with 
```bash
mpiexec -n num_processes python3 test.py instance_file params
```
- `num_processes`: number of processes to run the program using MPI, program needs at least 2 (1 master, and 1 worker process) to be used.
- `instance_file`: A file containing the graph (in edge list format).
- `params`: The parameter file used to configure the solver.
...
For easiest overview into how the wrapper is setup and run in code, please see `test.py` file.
...
*(Default values can be found in `biqbin.h`)*
