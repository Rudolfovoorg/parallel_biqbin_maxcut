...
class SolutionMaxCut(PrettyPrint):
    ...
    def __str__(self) -> str:
        return f'{super().__str__()}\nSolution MaxCut = {self.__solution}'
...
class SolutionQubo(SolutionMaxCut):
    ...
    def __str__(self):
        return (
            f'{super().__str__()}\n'
            f'Solution QUBO = {self.__solution}\n'
...
class ToFile(ABC, PrettyPrint):
    def __init__(self, filename: str, overwrite: bool = False):
        self.filename: str = filename
        self.overwrite: bool = overwrite

    @abstractmethod
    def write(self, solution: SolutionMaxCut) -> None:
        ...
    ...
    def write(self, solution: SolutionMaxCut, with_metadata=True) -> None:
        ...
        with open(output_path, 'w') as f:
            json.dump(save_output, f,
                      default=self._convert_numpy_to_json_serializable)
...
class QUBOSolver(MaxCutSolver[ProblemQubo]):
    ...
    def run(self) -> SolutionQubo | None:
        ...
        if self.get_rank() == 0:
            ...
            # biqbin_result['maxcut']['computed_val'] *= self.gcd
            # biqbin_result['meta_data']['parameters']['optimize_input'] = self.optimize_input
            # biqbin_result['meta_data']['parameters']['gcd'] = self.gcd
            biqbin_result['meta_data']['instance'] = self.problem.problem_name
            biqbin_result['meta_data']['parameters'] = {
                'time_limit': self.time_limit if self.time_limit > 0 else None
            }
