...
class ProblemBQP(ProblemMaxCut):
    ...
    def __init__(self, F: np.ndarray, c: np.ndarray, A: np.ndarray, b: np.ndarray, problem_name: str, offset: int = 0, optimize_input: bool = False, optimum_lol: float | None = None):
        """
...
class SolutionBQP(SolutionMaxCut):
    ...
    def get_bqp_solution(self, biqbin_result: dict,
        ...
        for candidate in (mc_x, 1 - mc_x):
            ...
            if np.isclose(objective, bqp_obj_value):
                ...
                bqp_result['optimum'] = problem.optimum
                bqp_result['opt_test'] = problem.optimum == bqp_result['computed_val']
                return bqp_result
    ...
    def __str__(self) -> str:
        return (f'{super().__str__()}\n'
                ...
                f'     Is optimum = {self.__solution['opt_test']}\n')
...
class BQPFromQBOSparse(FromFile):
    def read(self):
        ...
        optimum_lol = data['optimum']
        return ProblemBQP(F, c, A.toarray(), b, self.problem_name, offset, self.optimize_input, optimum_lol)
...
if __name__ == '__main__':
    ...
    fajlridr = BQPFromQBOSparse(args.problem_instance,
                                optimize_input=args.optimize)
    problem = fajlridr.read()
    ...
    if get_rank() == 0:
        ...
        print(solution.problem.problem_name,'-----', solution.solution['opt_test'])
