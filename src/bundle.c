 ...
 *       lambda >= 0, z >=0 , y free
 */
void solve_lambda(const int k, const double *Q, const double *c, double *out_lambda)
{
    ...
    double alpha = 1.0;
    double beta = 0.0;
    dsymv_(&UPLO, &k, &alpha, Q, &k, out_lambda, &inc, &beta, Qlam, &inc);
    ...
    double mu = ddot_(&k, z, &inc, out_lambda, &inc) / k;
    mu *= 0.5;
    ...
    double res_p = 1.0 - ddot_(&k, &e, &inc_e, out_lambda, &inc);

    // initial dual cost: y - 1/2*lambda'*Qlam
    temp = ddot_(&k, out_lambda, &inc, Qlam, &inc);
    double D_cost = y - 0.5 * temp;

    // intial primal cost: lambda'*c + 1/2 *lambda'*Qlam
    double P_cost = ddot_(&k, out_lambda, &inc, c, &inc) + 0.5 * temp;
    ...
    while (ABS(gap) > 1e-5)
    {
         ...
         */
        for (int i = 0; i < k; ++i)
            dz[i] = 1.0 / out_lambda[i] * (mu - out_lambda[i] * z[i] - z[i] * dlambda[i]);
         ...
         */
        y += alpha_d * dy;
        daxpy_(&k, &alpha_p, dlambda, &inc, out_lambda, &inc);
        ...
        // primal residual
        res_p = 1 - ddot_(&k, out_lambda, &inc, &e, &inc_e);
         ...
         * mu = .4*mu
         */
        mu = ddot_(&k, out_lambda, &inc, z, &inc) / k;
        mu *= 0.4;
        ...
        P_cost = ddot_(&k, out_lambda, &inc, c, &inc) + 0.5 * temp;
